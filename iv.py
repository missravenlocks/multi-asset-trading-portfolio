...
# Derive Weights using Full available history per asset
train_rets = strategy_returns.loc[:'2019-12-31']

# .std() in pandas ignores NaNs by default. 
# This calculates the volatility of QQQ since 2001 and BWX since 2007 accurately.
vols = train_rets.std() * np.sqrt(252)
inv_vols = 1.0 / (vols + 1e-10)
derived_w = inv_vols / inv_vols.sum()

# 3. Apply Weight Capping
weight_cap = 0.05
for _ in range(100):
    excess = derived_w[derived_w > weight_cap].sum() - (len(derived_w[derived_w > weight_cap]) * weight_cap)
    if excess <= 1e-10: break
    derived_w[derived_w > weight_cap] = weight_cap
    derived_w[derived_w < weight_cap] = derived_w[derived_w < weight_cap] + (excess * derived_w[derived_w < weight_cap] / derived_w[derived_w < weight_cap].sum())

print("--- Derived Inverse Volatility weights from the train period (pre-2020) ---")
print("--- Verify that these align with the hardcoded weights in the code  ---")
print(derived_w)

# Hardcoded Inverse Volatility Weights (provided in previous step)
# The combination strategy will be run using these hardcoded weights, not
# the derived weights above (though these should align)
HARDCODED_W = {
    'BWX':    0.050000,
    'DBC':    0.050000,
    'EWJ':    0.050000,
...
def calculate_portfolio(returns_df, weights_series):
    ...
    # Re-normalize: If Asset A (weight 0.1) is missing, 
    # we divide the daily return by (1.0 - 0.1) to keep the target exposure
    active_weights_sum = available_mask.dot(w)
    
    # Return the re-normalized portfolio return
    return daily_weighted_rets / active_weights_sum

# Apply the fix
strategy_returns['InvVol_Portfolio'] = calculate_portfolio(strategy_returns, hardcoded_w)
