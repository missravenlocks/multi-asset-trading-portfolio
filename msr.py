...
# Calculate Mu and Cov based on all available data per asset
# .mean() ignores NaNs, calculating the mean return of the asset's specific history
mu = train_rets.mean() * 252 

# .cov() calculates pairwise covariance for the overlapping periods of each pair
...
def objective(w):
    ...
    return - (p_ret / (p_vol + 1e-10)) # Avoid div by zero

# Optimization logic remains the same
cons = ({'type': 'eq', 'fun': lambda x: np.sum(x) - 1.0})
bnds = [(0.0, 0.05) for _ in range(num_assets)]
res = minimize(objective, np.array([1/num_assets] * num_assets), method='SLSQP', bounds=bnds, constraints=cons)
derived_w = pd.Series(res.x, index=mu.index)

print("--- Derived Max Sharpe Ratio weights from the train period (pre-2020) ---")
print("--- Verify that these align with the hardcoded weights in the code  ---")
print(derived_w)

# Hardcoded Max Sharpe Ratio Weights (provided in previous step)
...
# Apply the fix
strategy_returns['MSR_Portfolio'] = calculate_portfolio(strategy_returns, hardcoded_w)
